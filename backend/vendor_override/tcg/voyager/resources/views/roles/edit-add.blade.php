                    ...
                    <form class="form-edit-add" role="form"
                        ...
                        <div class="panel-body">
                            ...
                            <ul class="permissions checkbox">
                                ...
                                ?>
                                @foreach(Voyager::model('Permission')->getByUserLogin() as $table => $permission)
                                    <li>
                                        <input type="checkbox" id="{{$table}}" class="permission-group">
                                        <?php
                                            $model = (empty($table)) ? '' : __('voyager::generic.'. $table);
                                        ?>
                                        <label for="{{$table}}"><strong>{{  $model }}</strong></label>
                                        <ul>
                                            @foreach($permission as $perm)
                                                <li>
                                                    ...
                                                    <?php
                                                        $keyArray = explode('_',$perm->key);
                                                        $operation = __('voyager::generic.'. $keyArray[0]);
                                                        $model = (empty($table)) ? $keyArray[1] : $table;
                                                    ?>
                                                    <label for="permission-{{$perm->id}}">{{ $operation . ' '. __('voyager::generic.'. $model) }}</label>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </li>
